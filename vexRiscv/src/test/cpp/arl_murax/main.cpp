...
public:
	...
	GPIO(uint32_t *write, uint32_t *write_enable, uint32_t *read, uint32_t TimeRate){
		this->write = write;
...
public:
	MuraxWorkspace() : Workspace("Murax"){
		...
		GPIO *gpioA = new GPIO(&top->io_gpioA_read, &top->io_gpioA_writeEnable, &top->io_gpioA_write, 100000);
